...
class ZerobusStream:
    ...
    def __create_stream(self):
        ...
        if self._options.token_factory is not None:
            headers.append(("authorization", f"Bearer {self._options.token_factory()}"))
            if self._options.unity_catalog_url is not None:
                headers.append(("unity-catalog-endpoint", self._options.unity_catalog_url))
...
class ZerobusSdk:
    ...
    def create_stream(
        ...
        if self.__host.startswith(("[::1]", "http://localhost", "http://[::1]")):
            channel = grpc.insecure_channel(self.__host)
        else:
            channel = grpc.secure_channel(self.__host, grpc.ssl_channel_credentials())

        stub = zerobus_service_pb2_grpc.ZerobusStub(channel)
        stream = ZerobusStream(stub, table_properties, options, channel)
        stream._initialize()
        return stream
    ...
    def recreate_stream(self, old_stream: ZerobusStream) -> ZerobusStream:
        ...
        new_stream = self.create_stream(old_stream._table_properties, old_stream._options)
